...
<#
  ...
  install.ps1 directly, in a fixed, explicit order:
    ...
    4. wezterm, windows-terminal, atac, python, rust, zig, java, yazi,
       ...
       other. windows-terminal no-ops on Linux (nothing to configure) and
       tmux no-ops on Windows (no native build); neovim is last because it
       depends on both mise (Stage 2) and git (Stage 3).
...
$softStages = [ordered]@{
  ...
  "tmux"             = { & (Join-Path $RepoRoot "tmux" "install.ps1") }     # Linux-only; no-ops on Windows
