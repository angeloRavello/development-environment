...
2. `bootstrap.ps1` (pwsh7, both OSes), via `Invoke-Stage` for every step:
   ...
   2. **mise** — hard dependency (no `-ContinueOnError`): atac/gh/python/rust/zig/java/yazi/tmux/neovim all need it, so a failure here throws and aborts the whole script instead of cascading into confusing downstream failures.
